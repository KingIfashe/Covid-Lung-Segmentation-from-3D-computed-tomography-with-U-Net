...
def combined_focal_dice_loss(y_true, y_pred, focal_loss_alpha=0.8, focal_loss_gamma=2.0, dice_loss_weight=0.2):
    # Binary Focal Loss
    focal_loss = BinaryFocalCrossentropy(from_logits=False, alpha=focal_loss_alpha, gamma=focal_loss_gamma)(y_true, y_pred)
    
    # Dice Loss
    dice_loss_value = dice_loss(y_true, y_pred)
    
    # Combined Loss
    combined_loss = (1 - dice_loss_weight) * focal_loss + dice_loss_weight * dice_loss_value
...
model.compile(optimizer='adam', 
              loss=lambda y_true, y_pred: combined_focal_dice_loss(y_true, y_pred, 
...
model.save('my_model_focalloss.keras')
...
pred_msk = np.where(predictions>0.05,1,0)
